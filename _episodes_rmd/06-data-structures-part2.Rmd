...
`gapminder` dataset like we downloaded previously, but this time, we'll use the
...
> > ```{r eval = FALSE}
> > library(readxl)
> > gapminder <- read_excel(file = "data/gapminder_data.xlsx")
> > gapminder$country <- factor(gapminder$country)
> > gapminder$continent <- factor(gapminder$continent)
> > ```
> > Once the script is ready, save it in `scripts/` as `load-gapminder.R`.
> > 
